...
unsigned long sbi_sm_get_cert_chain_and_lak(uintptr_t cert_sm, uintptr_t cert_root, uintptr_t cert_man, uintptr_t cert_lak, int *lengths, unsigned int eid) {
  unsigned long ret;
  int sizes[4];
  ...
  ret = copy_cert_lengths_from_sm(sizes, (uintptr_t) lengths, 4 * sizeof(int));
  if (ret) {
